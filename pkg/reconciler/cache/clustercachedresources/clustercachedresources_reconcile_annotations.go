...
const (
	// AnnotationResourceKind is set on ClusterCachedResource objects to record the Kind of the cached resource.
...
// reconcileResourceMetadata ensures the ClusterCachedResource carries Kind and Scope annotations.
...
func (r *reconcileResourceMetadata) reconcile(ctx context.Context, clusterCachedResource *cachev1alpha1.ClusterCachedResource) (reconcileStatus, error) {
	...
	annotations := clusterCachedResource.Annotations
	if annotations == nil {
	...
	clusterCachedResource.SetAnnotations(annotations)
