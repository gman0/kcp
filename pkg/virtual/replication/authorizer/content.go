...
type contentAuthorizer struct {
	...
	getAPIExportsVirtualResourceIdentityAndGR func(vrIdentity string, gr schema.GroupResource) ([]*apisv1alpha2.APIExport, error)
...
func (a *contentAuthorizer) Authorize(ctx context.Context, attr authorizer.Attributes) (authorizer.Decision, string, error) {
	...
	if targetCluster.Wildcard {
		// We have a wildcard request. We need to check all relevant APIExports.
		exports, err = a.getAPIExportsVirtualResourceIdentityAndGR(cachedResource.Status.IdentityHash, schema.GroupResource{
	...
	// Make sure the user has apiexports/content permissions to the exports that export this CachedResource.
