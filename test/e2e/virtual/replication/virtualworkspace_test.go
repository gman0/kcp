...
func TestCachedResourceVirtualWorkspace(t *testing.T) {
	...
	t.Logf("=== get ===")
	{
		...
		_, err = wwUser1CachedResourceVWConfig.Cluster(consumerPath).WildwestV1alpha1().Cowboys("default").
	...
	t.Logf("=== list ===")
	{
		...
		kcptestinghelpers.Eventually(t, func() (bool, string) {
			var err error
			cowboyList, err = wwUser1CachedResourceVWConfig.Cluster(consumerPath).WildwestV1alpha1().Cowboys("default").
				List(ctx, metav1.ListOptions{})
			if apierrors.IsForbidden(err) {
				return false, fmt.Sprintf("waiting until rbac cache is primed: %v", err)
			}
	...
	t.Logf("=== watch ===")
	{
		...
		kcptestinghelpers.Eventually(t, func() (bool, string) {
			var err error
			cowboyWatch, err = wwUser1CachedResourceVWConfig.Cluster(consumerPath).WildwestV1alpha1().Cowboys("default").Watch(ctx, metav1.ListOptions{
